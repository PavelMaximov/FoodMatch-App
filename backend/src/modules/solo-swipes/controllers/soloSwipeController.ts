...
export class SoloSwipeController {
  ...
  async create(req: AuthRequest, res: Response) { const session=await service.createSession(req.user!.id, req.body?.filter ?? req.body ?? {}, { startOver: req.body?.startOver === true || req.body?.replaceExisting === true });res.status(session.resumedExisting?200:201).json({ session }); }
