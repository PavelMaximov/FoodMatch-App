...
export class LastFilterPresetService {
 async getLast(userId:string, mode:'solo'|'paired') {
  ...
  const currentUserPreset = await LastFilterPresetModel.findOne(scope).lean();
  if (currentUserPreset || mode === 'solo') return currentUserPreset;
  return LastFilterPresetModel.findOne({ mode, userId: null, pairKey: scope.pairKey }).sort({ usedAt: -1, updatedAt: -1 }).lean();
 }
