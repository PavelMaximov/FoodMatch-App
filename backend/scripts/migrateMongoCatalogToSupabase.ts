...
async function main(){
  ...
  const spiceLevelWarnings=dishes.flatMap(dish=>dish.warnings.map(warning=>({dishId:dish.legacyId,slug:dish.scalar.slug??null,name:dish.scalar.name,...warning})));
  const spiceLevelSummary={counts:dishes.reduce((counts:Record<string,number>,dish)=>(counts[dish.scalar.spice_level]=(counts[dish.scalar.spice_level]??0)+1,counts),{}),examples:dishes.slice(0,10).map(dish=>({dishId:dish.legacyId,slug:dish.scalar.slug??null,name:dish.scalar.name,spice_level:dish.scalar.spice_level}))};
  const report:any={startedAt:new Date(started).toISOString(),dryRun:options.dryRun,read:source.length,migrated:0,failed:[],unsupportedFields:unsupported,spiceLevelWarnings,spiceLevelSummary,reconciliations:[],duplicateDishCandidates:[],staleSupabaseCatalogRows:[],staleScanPerformed:fullSourceScope};
