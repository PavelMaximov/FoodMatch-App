...
export class LastFilterPresetController { async get(req:AuthRequest,res:Response){ const mode = req.query.mode === 'paired' ? 'paired' : 'solo'; const preset = await service.getLast(req.userId!, mode); res.json({preset}); } async put(req:AuthRequest,res:Response){ const mode = req.body?.mode === 'paired' ? 'paired' : req.body?.mode === 'solo' ? 'solo' : null; if(!mode) throw new AppError('Invalid filter mode',400); const preset = await service.saveLast(req.userId!, {...req.body, mode}); res.json({preset}); }}
