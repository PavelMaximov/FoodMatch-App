...
import { canonicalize, fieldReport, loadSource, migrateDish, parseOptions } from './catalogMigrationCore';
...
async function main(){
  ...
  try {for(const row of source){const dish=canonicalize(row);try{await migrateDish(db,dish,cache);report.migrated++;if(options.verbose)console.log(`[catalog] ${report.migrated}/${source.length} ${dish.scalar.slug??dish.legacyId}`);}catch(error){const failure={id:dish.legacyId,slug:dish.scalar.slug,name:dish.scalar.name,error:error instanceof Error?error.message:String(error)};report.failed.push(failure);console.error('[catalog] rolled back dish',failure);if(options.failFast)break;}}}
